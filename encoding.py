...
# 9. Count Encoding
# Use Case:
# - Counting Occurrences: Count of occurrence of each category is more significant than categorical value.
count_encoding = df['Fruit'].value_counts().to_dict()
df['Fruit_Count_Encoded'] = df['Fruit'].map(count_encoding)
print("Count Encoding:\n", df[['Fruit', 'Fruit_Count_Encoded']], "\n")